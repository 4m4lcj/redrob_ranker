...
def check_open_to_work_salary_honeypot(candidate: dict) -> bool:
    ...
    if sal_max is not None and sal_max < 5.0:
        return False
...
def check_signals_hard(candidate: dict) -> tuple[bool, str]:
    ...
    if (not signals.get("open_to_work_flag", True) and days_inactive > 90 and (signals.get("recruiter_response_rate") or 0) < 0.15):
...
def apply_filters(candidate: dict) -> tuple[bool, str]:
    ...
    if yoe < 4:
        return False, f"yoe_too_low: {yoe:.1f} < 3"
    if yoe > 20:
    ...
    has_core = any(core in name or name in core for name in cand_skill_names for core in CORE_AI_SKILLS)
